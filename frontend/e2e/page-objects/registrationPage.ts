...
export class RegistrationPage {
    ...
    async fillInRegistrationFormAndClickCreateButton(email?: string, password?: string, name?: string, phone?: string, pin?: string) {
        if (email) await this.page.getByLabel('Your email').fill(email);
        if (password) await this.page.getByLabel('Password', { exact: true }).fill(password);
        if (password) await this.page.getByLabel('Confirm password').fill(password);
        if (name) await this.page.getByPlaceholder('Your Name').fill(name);
        if (phone) await this.page.getByLabel('Phone Number').fill(phone);
        if (pin) await this.page.getByLabel('Create Wallet pin').fill(pin);
